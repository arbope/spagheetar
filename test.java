	...
	public class test{
		...
		static final String[] NOMBRES = new String[] {
			"Tónica (1)","Segunda menor (b2)","Segunda mayor (2)","Tercera menor (b3)","Tercera mayor (3)",
			"Cuarta justa (4)","Tritono (#4/b5)","Quinta justa (5)","Sexta menor (b6)","Sexta mayor (6)",
			"Séptima menor (b7)","Séptima mayor (7)"
		};
		...
		static void parseTuning(String tuning){
			...
			for (int i= 0; i < stringNotation.length; i++){
			    System.out.print(stringNotation[strings - i - 1]);
			}	
		}
					...
					for (int j = 0; j < none.length; j++){
						if (n == none[j] && !printed){
							if (notes[(n + keyShift) % 12].length() == 2)						
								...
								System.out.print(COLORS[n % 12] + notes[(n + keyShift) % 12] + RESET +  "_|");
							printed = true;
						} 
					}
					if (!printed){
		...
		static void askModeAndKey(Scanner sc){
			//ASKS MODE 
			System.out.println("Select mode! \n MAJOR / MINOR / MINOR_ARM / MINOR_MEL / EXIT");
			mode = sc.nextLine();
			if (mode.equals("EXIT")){
				System.exit(0);
			}
